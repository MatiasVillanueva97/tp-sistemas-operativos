...
int main(int argc, char* argv[]) {
	...
	while(1) {  // main accept() loop
	 ...
	 if (new_fd= accept(listener, (struct sockaddr *)&their_addr, &sin_size) == -1) {
		 perror("accept");
	 continue;
	 }
	 inet_ntop(their_addr.ss_family,getSin_Addr((struct sockaddr *)&their_addr),s, sizeof s); // para poder imprimir la ip del server
